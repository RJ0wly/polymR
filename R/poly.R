...
#' degree.
#' @usage polynomial_features(dataframe,degree)
...
#' df_poly = polynomialFeatures(df,2)
#' @export
polynomial_features <- function(df,degree){
tmp =  vector()
cpt = 1
len = degree*length(df)
while(cpt < len){
  for(j in colnames(t)){
    for(i in seq_along(1:degree)){
      tmp[[cpt]] = paste0('poly(',j,',',degree,',raw=TRUE)[,',i,']')
      cpt = cpt + 1
    }
  }
}
